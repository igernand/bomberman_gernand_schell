...
class BombeRLeWorld(GenericWorld):
    ...
    def new_round(self):
        ...
        # coding crates here
        self.arena[:,:] = 0 #hardcode "floor"
        self.arena[1,1] = 1 #hardcode "crates"
            ...
            for i in range(coin_pattern.shape[0]):
                for j in range(coin_pattern.shape[1]):
                    if coin_pattern[i, j] == 1:
                        ...
                        coins[x + i, x + j] += 1"""
        """if self.arena[1,1] == 0:
            self.coins.append(Coin((1, 1)))
            self.coins[-1].collectable = True"""
        """if self.arena[9,9] == 0:
            self.coins.append(Coin((9, 9)))
            self.coins[-1].collectable = True
        if self.arena[5,5] == 0:
            self.coins.append(Coin((5, 5)))
            self.coins[-1].collectable = True
        if self.arena[11,3] == 0:
            self.coins.append(Coin((11, 3)))
            self.coins[-1].collectable = True"""
        ...
        for agent in self.agents:
            ...
            #agent.x, agent.y = (15,1)
            if (agent.x, agent.y) == (1,1):
                agent.x, agent.y = start_positions.pop()
