...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if new_game_state["step"] != 1:
        ...
        #old_q_value = self.qtable[features_old][self_action]
        ...
        #temp_diff = reward_from_events(self, events) + GAMMA * np.max(self.qtable[state_to_features(new_game_state)]) - old_q_value
            
        # update q-value 
        new_q_value = old_q_value + (ALPHA * temp_diff)
        #self.qtable[features_old][self_action] = new_q_value
