...
import numpy as np
# first master agent
# already a little bit improved for higher crate density, 
# therefore not perfect for low crate density anymore, but still good
...
def act(self, game_state: dict) -> str:
    ...
    for k in range(len(crates_x)):
        ...
        y = crates_y[k]
        if x%2==1 and y%2==1: 
            self.qtable = self.qtable + self.crate1table[(15-y):(30-y), (15-x):(30-x)]
        
        elif x%2==0 and y%2==1:
            self.qtable = self.qtable + self.crate2table[(15-y):(30-y), (14-x):(29-x)]
        
        elif x%2==1 and y%2==0:
            self.qtable = self.qtable + self.crate3table[(14-y):(29-y), (15-x):(30-x)]
  
        if bomb_off == 0:
            add_bombtable(x, y, self)
    ...
    if board[x_self, y_self-1]!=0:
        self.qtable[y_self -1, x_self -1, 0]= -10000
    if board[x_self, y_self+1]!=0:
        self.qtable[y_self -1, x_self -1, 2]= -10000
    if board[x_self+1, y_self]!=0:
        self.qtable[y_self -1, x_self -1, 1]= -10000
    if board[x_self-1, y_self]!=0:
        self.qtable[y_self -1, x_self -1, 3]= -10000
