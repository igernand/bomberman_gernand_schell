...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if new_game_state["step"] != 1:
        ...
        old_q_value = self.timertable[y_old][x_old][self_action]
        temp_diff = reward_from_events(self, events) + GAMMA * np.max(self.timertable[y][x][0:6]) - old_q_value
        new_q_value = old_q_value + (ALPHA * temp_diff)
        self.timertable[y_old][x_old][self_action] = new_q_value
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    if last_game_state["step"] != 1:
        ...
        old_q_value = self.timertable[y_old][x_old][last_action]
        temp_diff = reward_from_events(self, events) + GAMMA * np.max(self.timertable[y][x][0:6]) - old_q_value
        new_q_value = old_q_value + (ALPHA * temp_diff)
        self.timertable[y_old][x_old][last_action] = new_q_value
    
    # Store the q-table
    with open("my-saved-timertable.pt", "wb") as file:
        pickle.dump(self.timertable, file)
